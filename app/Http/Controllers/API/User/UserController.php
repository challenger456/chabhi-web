    ...
    public function socialLogin(Request $request)
{
    ...
    if($input['login_type'] === 'mobile'){
        $user = User::where('username',$input['username'])->where('login_type','mobile')->first();
    } else {
        $user = User::where('email',$input['email'])->first();
    }

    if($user != null) {
        if(!isset($user->login_type) || $user->login_type == ''){
        ...
        $trashed_user = User::where($key,$value)->whereNotNull('login_type')->withTrashed()->first();
    ...
    if($user) {
        ...
        if($user->login_type !== null && $user->login_type !== 'mobile'){
            $user->profile_image = $user->social_image ? $user->social_image : getSingleMedia($user, 'profile_image', null);
        } else {
            $user->profile_image = $user->profile_image ? $user->profile_image : getSingleMedia($user, 'profile_image', null);
        }
    ...
    return comman_message_response(__('messages.something_wrong'), 400);
}
